...
class DatasetManagerDialog(QDialog):
    ...
    def __init__(self, view_sequence_signal=None, parent=None):
        super().__init__(parent)
        self.view_sequence_signal = view_sequence_signal
    ...
    def parse_sequence_file(self, file_path: str) -> Optional[DatasetItem]:
        ...
        item.is_aligned = len(set(lengths)) == 1
    ...
    def view_dataset(self, row: int):
        ...
        if 0 <= row < len(self.dataset_items):
            ...
            # 使用SeqViewer插件查看序列
            if self.view_sequence_signal:
                self.view_sequence_signal.emit(item.sequences)
            else:
                # 备用方案：显示基本信息
                QMessageBox.information(
                    self, "View Dataset", 
                    f"Loci: {item.loci_name}\n"
                    f"Sequences: {item.sequence_count}\n"
                    f"Length: {item.length}\n"
                    f"Aligned: {'Yes' if item.is_aligned else 'No'}"
                )
